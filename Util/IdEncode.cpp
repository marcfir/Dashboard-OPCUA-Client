...
static inline bool requireEncoding(char c) { return !isalnum(c) && (Whitelist.find(c) == Whitelist.end()); }
...
std::string IdEncode(const std::string &id) {
  ...
  for (const auto &c : id) {
    if (requireEncoding(c)) {
      ss << "_" << std::uppercase << std::setw(2) << std::setfill('0') << static_cast<int>(c) << std::nouppercase;
    } else {
      ss << c;
    }
  }
