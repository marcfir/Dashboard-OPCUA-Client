...
class TestOpcUaClient : public OpcUaClient {
 public:
  ...
  void callUpdateNamespaceCache() { updateNamespaceCache(); }
  ...
  static std::shared_ptr<Umati::OpcUa::MockOpcUaWrapper> getWrapper() {
    ...
    EXPECT_CALL(*mockWrapper, DiscoveryGetEndpoints).Times(AtLeast(1)).WillRepeatedly(DoAll(SetArgReferee<3>(endpointDescriptions), Return(uaStatus1)));
...
TEST(OpcUaClient, TranslateBrowsePathToNodeId) {
  ...
  Umati::OpcUa::TestOpcUaClient client(
    std::string("someUrl"), std::string("someUser"), std::string("somePassword"), OpcUa_MessageSecurityMode_None, mockWrapper);
...
TEST(OpcUaClient, updateNamespaceCache_updates) {
  ...
  EXPECT_CALL(*mockWrapper, SessionGetNamespaceTable)
    .Times(AtLeast(2))
...
TEST(OpcUaClient, Browse) {
  ...
  OpcUa_ReferenceDescription opcUaReferenceDescriptions[1]{OpcUa_ReferenceDescription()};

  UaReferenceDescriptions referenceDescriptions(1, opcUaReferenceDescriptions);

  EXPECT_CALL(*mockWrapper, SessionRead).Times(AtLeast(1)).WillRepeatedly(DoAll(SetArgReferee<4>(readResult), Return(good)));
  EXPECT_CALL(*mockWrapper, SessionBrowse).Times(AtLeast(1)).WillRepeatedly(DoAll(SetArgReferee<4>(referenceDescriptions), Return(good)));
